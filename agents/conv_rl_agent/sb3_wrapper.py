...
from luxai_s2.unit import BidActionType, FactoryPlacementActionType, UnitType
...
class SB3Wrapper(gym.Wrapper):
    ...
    def reset(self, **kwargs):
        ...
        action = {
            agent: dict() for agent in self.env.agents
        }

        for agent in self.env.agents:
            for i in range(self._max_agent_robots):
                for factory in self.env.state.factories[agent]:
                    self.env.add_unit(
                        self.env.state.teams[agent],
                        UnitType.HEAVY,
                        self.walk_around(
                            self.env.state.factories[agent][factory].pos, i).pos,
                    )
    ...
    def walk_around(self, pos, steps):
        neighbours = [
            [2, 1],
            [0, 0],
            [0, -1],
        ...
        return pos + neighbours[steps % len(neighbours)] * (steps // len(neighbours) + 1)
